...
export default function ManilaFolder({
  ...
  return (
    <div
      onClick={onClick}
      className={`relative w-full max-w-75 h-55 cursor-pointer group transition-all duration-300 hover:scale-105 ${rotation} ${className}`}
      role="button"
    ...
    >
      ...
      <div className="absolute top-6 left-4 right-4 h-45 bg-white shadow-sm transform -rotate-1 transition-transform duration-300 group-hover:-translate-y-4 border border-zinc-200">
      ...
      <div className="absolute top-10 left-0 w-full h-[calc(100%-2.5rem)] bg-[#f3e5ab] rounded-b-lg shadow-lg border-t border-l border-r border-b border-[#d8c68a] flex flex-col items-center justify-center transition-transform origin-bottom duration-300 group-hover:rotate-x-6">
